...
CameraController::CameraController(const int &layer, const std::string &script) : EntitySystem(layer, script) { }

CameraController::~CameraController() { }

void CameraController::Init() {
	//third_person_.reference = vec3(0, 3.0f, 3.0f);
	third_person_.reference = vec3(0, 0, 0);
	third_person_.up = vec3(0, 1, 0);

	//top_down_.reference = vec3(0, 3.0f, 0);
	top_down_.reference = vec3(0, 0, 0);
...
void CameraController::ReloadScript() {
	EntitySystem::ReloadScript();

	float up = GetAttribute<float>("up");
	float back = GetAttribute<float>("back");

	third_person_.reference.y = up;
	third_person_.reference.z = back;

	top_down_.reference.y = up;
